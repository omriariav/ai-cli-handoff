...
Plugin records with a local Claude install cache are bridged directly: `ai-handoff` copies the plugin into `~/.codex/plugins/cc-<name>`, strips Claude-only `hooks/`, `commands/`, and `agents/`, converts `agents/*.md` into Codex TOML under `~/.codex/agents`, writes an `x-cc-bridge` marker, updates `~/.agents/plugins/marketplace.json`, and then runs `codex plugin add cc-<name>@cc-bridged-plugins` after explicit Codex-wide apply confirmation. Before bridging, it inspects the plugin origin from Claude marketplace metadata, local git remotes, and cache paths; if a native `.codex-plugin/plugin.json` is found locally, the candidate is marked `codex-native`. GitHub origin checks are on by default for both `globals` and the interactive `g` picker: the CLI uses authenticated `gh` (`gh auth status`, then `gh api`) to check the repo's native `.codex-plugin/plugin.json`. If `gh` is missing, unauthenticated, or the API check fails, it prints a clear GitHub check failure and keeps the bridge/manual fallback. Use `--no-check-github` for a fully offline listing. After installing a bridged plugin, restart Codex or open a new session.

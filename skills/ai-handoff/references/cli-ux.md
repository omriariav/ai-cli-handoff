...
`ai-handoff <path>` is optimized for a one-time handoff. It should read as a wizard, not a dashboard. The wizard starts with a handoff direction choice:
...
2. Project files: show the exact project-local files, offer preview diff, include in the final plan, skip, or quit. This step queues the project files only; it does not write.
3. Tooling and Claude setup carryover: print the `Step 3/4` header before any slow compatibility checks, then show bounded progress lines so the user knows scanning and GitHub/native-Codex checks are running. Summarize conversation-matched MCPs, skills, and plugins first under `Detected from selected conversations`. Before offering installs, check the current Codex user-level plugin setup, including bridged plugins under `~/.codex/plugins`; already-available tools should be listed separately and not checked for reinstall. Do not open the detailed full-setup picker by default. Open a static checkbox picker for the remaining conversation-matched tools, default them to checked, support Space/number toggles, and let Enter commit the selected set before asking whether to record in project handoff only or install for this Codex user. Step 3 queues the tooling scope only; it does not write project files or change `~/.codex`. Broader Claude setup counts should be secondary and framed as `Expand to full Claude setup`, not as the main path. The expansion reviews everything configured in Claude, including tools not seen in the selected conversations.
4. Review and run plan: show `Step 4/4: Review & Run Plan` with the target project, flow, selected conversations, queued project-file writes, selected tooling and scope, already-available tooling, captured Claude setup, and artifact paths. The user can preview the diff, run, or quit. This final approval is the first point where the wizard may write project files or install user-level Codex tooling. If user-level installs are queued, blank Enter must not run them; require the user to type `install`.

Step 3 should also show captured-only Claude setup:

- Hooks: captured, review required, never enabled automatically because they can run commands.
- Rules: project guidance is merged into AGENTS.md; Claude permission rules are recorded for review.
- References: project-local and external paths are recorded; external references are not copied automatically.
- Statusline: captured only; Codex does not support Claude statusline rendering yet.

The wizard must keep installs behind a final review confirmation that explicitly says it can change `~/.codex` and make tooling available in every Codex project for this OS user. Codex CLI currently exposes these installs as user-level, not project-local; project-only means recording intent and context in AGENTS.md/manifest without installing.

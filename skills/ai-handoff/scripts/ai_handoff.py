...
def redacted_markdown_snapshot(text: Optional[str], limit: int = CLAUDE_MD_SOURCE_LIMIT) -> Dict[str, Any]:
    if not text:
        return {"text": "", "truncated": False, "limit": limit}
    redacted = str(redact(text))
    return {
        "text": truncate(redacted, limit),
...
def discover_project(project: Path) -> Dict[str, Any]:
    ...
    return {
        ...
        "claude_md_source": redacted_markdown_snapshot(claude_md),

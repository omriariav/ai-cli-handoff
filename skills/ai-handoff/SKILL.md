...
4. In the project files step, preview the diff or queue project-local writes for the final review:
   - `AGENTS.md`
   - `.codex/handoff/summary.md`
   - `.codex/handoff/manifest.json`
   - `.codex/handoff/runs/<run-id>.json`
5. In the tooling carryover step, print `Step 3/4` before slower checks, then show progress while scanning tooling and checking GitHub/native-Codex compatibility. Lead with actions that match tooling used in the selected conversations, explain bridge-vs-native decisions inline, and let the user pick matched tools in a static checkbox picker like the conversation picker. Default all conversation-detected tools to checked, support Space/number toggles, and use `e` to expand to full Claude setup. Check the current Codex user-level plugin setup first; tools already available under `~/.codex` should be shown as already available, not selected for reinstall. After selection, offer a simple scope/action choice: record in project handoff only, install for this Codex user, or quit. Step 3 records the choice only; it must not write project files or change `~/.codex`. Installing for this Codex user writes under `~/.codex` and makes the tooling available to every Codex project for that user, but only after the final plan review is approved. The broader Claude setup remains available through `expand full setup`.
...
6. In `Step 4/4: Review & Run Plan`, summarize the queued plan before anything runs: project path, handoff flow, selected conversation count, project files included or skipped, selected tooling count/scope, already-available tools, captured hooks/rules/references/statusline, and artifact paths. Offer diff preview, run, or quit. This is the first wizard step that may write project files or install Codex-wide tooling. If the plan includes installs for this Codex user, Enter must not execute them; require the user to type `install`.
7. Apply selected Codex-wide installs only after explicit confirmation in the final review. These can change `~/.codex` and affect every Codex project/folder on this machine:
   - MCP imports run selected `codex mcp add ...` commands.
   - Skill imports copy selected Claude skill folders into `~/.codex/skills`.
   - Plugin imports detect native Codex metadata from the source repo and report it for review, but `0.1.0` does not execute a native plugin install path automatically. Bridge from the source repo at the Claude-used ref when possible; use the installed Claude cache only as an explicit fallback when source resolution fails.
...
8. Mark the goal complete after the dry-run summary or apply artifacts have been produced. The completion screen should list selected conversation count, project files written, Codex-wide installs completed or recorded, and artifact paths to inspect.

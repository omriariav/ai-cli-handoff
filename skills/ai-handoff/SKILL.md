...
5. In the tooling carryover step, lead with actions that match tooling used in the selected conversations. Do not open the detailed picker by default. Offer a simple scope/action choice: record in project handoff only, install for this Codex user, customize exact items, skip, or quit. Installing for this Codex user writes under `~/.codex` and makes the tooling available to every Codex project for that user. The broader Claude inventory remains available through Customize views such as `all`.

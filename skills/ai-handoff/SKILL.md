...
4. In the project files step, preview the diff or queue project-local writes for the final review:
   ...
   - `AGENTS.md` should clearly say the section was generated by ai-handoff and include a first-run Codex ask: read selected conversations, captured Claude setup, and the redacted `CLAUDE.md` source snapshot; then write durable context outside the managed markers, merge rules from `CLAUDE.md`, and drop stale one-off handoff details.
...
7. Apply selected Codex-wide installs only after explicit confirmation in the final review. These can change `~/.codex` and affect every Codex project/folder on this machine:
   ...
   - Skill imports copy selected Claude skill folders into `~/.codex/skills`. In `0.3.0`, scan user-level `~/.claude/skills`, project-local `.claude/skills`, and project-local `.agents/skills`; resolve symlinked skill folders before copying; and record `npx skills add/install/update <skill>` commands from Claude settings as origin evidence without running them automatically.
   - Plugin imports detect native Codex metadata from the formal source and report it for review. In `0.2.0`, prefer local marketplace/source repos or remote GitHub sources over installed Claude cache copies. Bridge from the authoritative source when possible; use the installed Claude cache only as an explicit fallback when source resolution or GitHub materialization fails.
